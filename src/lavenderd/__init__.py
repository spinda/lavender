...
__version__ = lavender.__version__
...
# Response headers
LAVENDER_VERSION_HEADER = "X-Lavender-Version"
...
async def handle(request: web.Request, executor: Executor) -> web.Response:
    headers = {LAVENDER_VERSION_HEADER: __version__}
    try:
        ...
        if only_diff:
            ...
            formatted_str = await loop.run_in_executor(
                executor,
                partial(lavender.diff, req_str, formatted_str, src_name, dst_name),
            )
