...
# lib2to3 fork
from llib2to3.pytree import Node, Leaf, type_repr
from llib2to3 import pygram, pytree
from llib2to3.pgen2 import driver, token
...
def parse_pyproject_toml(path_config: str) -> Dict[str, Any]:
    """Parse a pyproject toml file, pulling out relevant parts for Lavender
...
@click.option(
    ...
    multiple=True,
    help=(
        "Python versions that should be supported by Lavender's output. [default: per-file"
        " auto-detection]"
    ),
)
@click.option(
...
def format_str(src_contents: str, *, mode: Mode) -> FileContent:
    ...
    >>> import lavender
    >>> print(lavender.format_str('def f(arg:str="")->None:...', mode=Mode()))
    def f(arg: str = '') -> None:
        ...
...
class LineGenerator(Visitor[Line]):
    ...
    def visit_INDENT(self, node: Leaf) -> Iterator[Line]:
        ...
        # In llib2to3 INDENT never holds comments.
...
class ProtoComment:
    ...
    It's not a :class:`llib2to3.pytree.Leaf` so that:
...
class StringMerger(CustomSplitMapMixin, StringTransformer):
    ...
    def __merge_string_group(self, line: Line, string_idx: int) -> TResult[Line]:
        ...
        BREAK_MARK = "@@@@@ LAVENDER BREAKPOINT MARKER @@@@@"
...
class BaseStringSplitter(StringTransformer):
    ...
    Requirements:
        * The target string value is responsible for the line going over the
        line length limit. It follows that after all of lavender's other line
...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    try:
        ...
        raise AssertionError(
            f"INTERNAL ERROR: Lavender produced invalid code: {exc}. Please report a bug"
            " on https://github.com/spinda/lavender/issues.  This invalid output might be"
            f" helpful: {log}"
        ) from None
    ...
    if src_ast_str != dst_ast_str:
        ...
        raise AssertionError(
            "INTERNAL ERROR: Lavender produced code that is not equivalent to the"
            " source.  Please report a bug on https://github.com/spinda/lavender/issues. "
...
def assert_stable(src: str, dst: str, mode: Mode) -> None:
    ...
    if dst != newdst:
        ...
        raise AssertionError(
            "INTERNAL ERROR: Lavender produced different code on the second pass of the"
            " formatter.  Please report a bug on https://github.com/spinda/lavender/issues."
            f"  This diff might be helpful: {log}"
        ) from None
